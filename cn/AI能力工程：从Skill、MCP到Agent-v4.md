...
| 返回内容 | 目的 |
|---|---|
| `summary` | 给 Agent 当前判断所需的简短事实 |
| `evidence_refs` | 指向页码、段落、表格、文件路径或 Resource URI |
| `confidence` | 标注抽取或匹配置信度，方便低置信度时升级 |
| `coverage` | 说明覆盖了哪些范围，哪些范围未处理 |
| `next_queries` | 给 Agent 后续可读取的相关片段或引用节点 |
...
1. 先识别文档类型和业务场景。
2. 再选择适用规则集。
3. 对金额、日期、期限、编号等确定字段用规则抽取。
4. 对责任分配、兜底义务、排他条款、隐含引用等模糊内容用 LLM 判断。
5. 每条风险都必须绑定证据位置。
6. 低置信度或高风险事项必须升级人工。
7. 输出未覆盖范围，不能假装全面审查。
...
- `ingest_document_package`
- `read_document_graph`
- `extract_clause_candidates`
- `search_reference_rules`
- `apply_rule_set`
...
1. ★ 说明为什么合同审查 Agent 不能直接把原始 PDF 全文交给 LLM。
2. ★★ 为“违约金过高”设计一组可配置判断标准，至少包含金额、比例、业务场景和升级条件。
3. ★★★ 设计一个招标审查 Agent 的文档图谱结构，覆盖章节、表格、附件、引用关系和证据定位。
...
2. ★★ 两个来源给出的净利润不一致时，按 20.6 写出处理流程。
...
1. ★ 判断你当前的 AI 项目处于 23.1-23.7 哪个阶段，并说明依据。

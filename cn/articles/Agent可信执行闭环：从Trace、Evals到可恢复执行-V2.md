...
- [Trace Event Schema](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/schemas/trace-event.md)
- [Checkpoint State Schema](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/schemas/checkpoint-state.yaml)
- [Eval Case Schema](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/schemas/eval-case.yaml)
- [Validator 决策表](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/checklists/validator-decision.md)
- [Regression Report 模板](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/checklists/regression-report.md)

## 二、可运行案例：代码修复 Agent

案例位于 [代码修复 Agent README](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/examples/code-repair-agent/README.md)。Runner 固定提供四个离线工具：`search_repo`、`run_tests`、`apply_patch` 和 `git_diff`。本 fixture 的工具只返回生产代码路径；通用 Runner 仍需在工具层实施路径 containment 和权限策略，具体边界可参考 [权限矩阵](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/checklists/permission-matrix.md)。
...
- [Agent 可观测性实战：从日志、Trace 到 Replay](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/articles/Agent可观测性实战：从日志、Trace到Replay.md) 讨论事件检索、成本与 Replay。
- [Agent 的可观测性实战：用 Tracing 看清你的 Agent“大脑”](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/articles/Agent的可观测性实战：用Tracing看清你的Agent“大脑”.md) 深入 `Trace -> Span -> Event` 和埋点属性。
- [Agent 的自动化评估体系（Evals）](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/articles/Agent的自动化评估体系（Evals）：从单元测试到集成评测.md) 讨论评估分层、Golden Set 和证据要求。
- [Agent 状态管理与断点续传](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/articles/Agent状态管理与断点续传：Checkpointer机制深度解析.md) 讨论游标、状态版本和幂等恢复。
- [Agent 自我纠错与验证机制](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/articles/Agent自我纠错与验证机制设计.md) 讨论 Validator、事实核查和失败归因。
...
失败不是把结果改写成“成功”，而是要留下可解释的分类、证据和下一步动作。Validator 决策细节见 [Validator 决策表](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/checklists/validator-decision.md)。
...
输出为 `regression-report.json` 和 `regression-report.md`。报告应能回答“哪个 Case 失败、失败发生在哪个事件、是否产生了禁止动作、从哪个 Checkpoint 恢复”。高风险样本必须阻断或转人工；成功率相对基线下降超过 5 个百分点，或成本/延迟回归达到 20%，发布决策为 `blocked`。报告格式见 [Regression Report 模板](https://github.com/MathsionYang/ai-capability-engineering/blob/main/cn/assets/checklists/regression-report.md)。Evals 用于证明是否发生回归，而不是预先保证所有 Case 通过。
